...
def log():
    ...
    if user_data is not None:
        ...
        if stored_password == password:
            CTkMessagebox(title="Info", message="Welcome!")
            emailglobal = log_email.get()
            passglobal = log_pwd.get()
            login.place_forget()
            home.place(relx=0.5, rely=0.5, anchor=tkinter.CENTER)
        else:
            CTkMessagebox(title="Info", message="Check Your Password.")
    else:
        CTkMessagebox(title="Info", message="Check Your E-mail.")
...
def cryptcaesaralpha():
    if msgalpha.get() == "" and msgalpha2.get() == "":
        CTkMessagebox(title="Cryptalpha", message=" Sir You must fill the form !!")
    else:
...
def cryptcaesarascii():
    if msgascii2.get() == "" and msgascii.get() == "":
        CTkMessagebox(title="Cryptascii", message="Sir You must fill the form !!")
    else:
...
def Dryptcaesarascii():
    if msgascii2.get() == "" and msgascii.get() == "":
        CTkMessagebox(title="Drypt", message="Sir You must fill the form !!")
    else:
